...
{
    private const string GetMovieRatings = "GetMovieRatings";
    private const string GetMovieRatingById = "GetRatingMovieById";
    private const string CreateMovieRating = "CreateMovieRating";
    private const string UpdateMovieRating = "UpdateMovieRating";
    private const string DeleteMovieRating = "DeleteMovieRating";
        ...
        [FromRoute] int movieId, [FromBody] CreateMovieRatingRequestBody requestBody, CancellationToken cancellationToken = default)
    {
        ...
        return TypedResults.CreatedAtRoute(endpointResult, GetMovieRatingById, new { ratingId = response.Id });
    }

    private static async Task<IResult> UpdateMovieRatingAsync(
        [FromServices] IRatingService ratingService, [FromServices] IRatingMapper ratingMapper, [FromServices] ILinkService linkService,
    ...
    {
        var request = new UpdateMovieRatingRequest( movieId, ratingId, requestBody.SourPopcorns, requestBody.Comment);
    ...
    private static async Task<IResult> DeleteMovieRatingAsync(
        [FromRoute] int movieId, [FromServices] IRatingService ratingService, [FromRoute] int ratingId, CancellationToken cancellationToken = default)
    {
        var request = new DeleteMovieRatingRequest(movieId, ratingId);
    ...
    {
        if (hasPrevious)
            yield return linkService.Generate(GetMovieRatings, new { movieId, pageNumber = pageNumber - 1, pageSize }, "self", "GET");
        if (hasNext)
            yield return linkService.Generate(GetMovieRatings, new { movieId, pageNumber = pageNumber + 1, pageSize }, "self", "GET");
    }
    ...
    {
        yield return linkService.Generate(UpdateMovieRating, new { movieId, ratingId }, "self", "PUT");
        yield return linkService.Generate(DeleteMovieRating, new { movieId, ratingId }, "self", "DELETE");
    }

    private static IEnumerable<Link> GenerateCreateLinks(ILinkService linkService, int movieId, int ratingId)
    {
        yield return linkService.Generate(GetMovieRatingById, new { movieId, ratingId }, "self", "GET");
        yield return linkService.Generate(UpdateMovieRating, new { movieId, ratingId }, "self", "PUT");
        yield return linkService.Generate(DeleteMovieRating, new { movieId, ratingId }, "self", "DELETE");
    }
